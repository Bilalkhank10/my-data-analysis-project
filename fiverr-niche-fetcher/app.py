...
class GenerationRunRequest(BaseModel):
    ...
    refinement_model: str | None = Field(default=None, max_length=200)
...
async def export_job_json(job_id: str) -> StreamingResponse:
    job = manager.get_job(job_id)
    if job is None:
        raise HTTPException(status_code=404, detail="Job not found")
    results, _ = storage.get_job_results(job_id, offset=0, limit=1000)
    data = json.dumps(results, ensure_ascii=False, indent=2)
    output = io.BytesIO(data.encode("utf-8"))
    return StreamingResponse(
        output,
        media_type="application/json",
        headers={"Content-Disposition": f'attachment; filename="fiverr_scrape_{job_id}.json"'}
    )
...
<body class="design-4" data-workspace="crawl">
<div class="d4-top">
    <div class="d4-brand"><span class="d4-mark">G</span> GigCraft Lab</div>
    <div class="d4-tabs">
      <a href="/"><span>Studio</span></a><span class="on">Lab</span>
    </div>
    <div class="d4-user"><span class="dot on" style="margin-right:6px; background:var(--ok); border-radius:50%; width:8px; height:8px; display:inline-block;"></span><span>Private workspace</span></div>
  </div>
  <div class="d4-shell">
    <div class="d4-side" id="workspaceNav">
      <div class="d4-group">Workspaces</div>
      <button type="button" class="d4-item on" data-workspace="crawl">◉ Crawl</button>
      <button type="button" class="d4-item" data-workspace="intelligence" disabled>◌ Intelligence</button>
      <button type="button" class="d4-item" data-workspace="ai" disabled>◌ Audit</button>
      <button type="button" class="d4-item" data-workspace="builder" disabled>◌ Builder</button>
      <button type="button" class="d4-item" data-workspace="raw" disabled>◌ Source</button>
    </div>
  ...
  <div class="stage">
    <main class="canvas wide wrap">
      ...
      <section id="analysisPanel" class="analysis-panel workspace-intelligence">
        <div class="analysis-head"><div><h2>Market intelligence</h2><p id="analysisMeta">Deterministic analytics · No LLM</p></div><div style="display:flex; gap:8px;"><a id="downloadRawJson" class="button" href="#" style="background: gold; color: #333; border: 1px solid goldenrod; font-weight: bold;">Download Full JSON</a><a id="analysisCsv" class="button secondary" href="#">Download CSV</a></div></div>
        <div id="analysisTabs" class="tabs">
          <button type="button" data-tab="overview" class="active">Overview</button>
          <button type="button" data-tab="health" class="on">Health ★</button>
      ...
      <section id="builderPanel" class="builder-panel workspace-builder">
        ...
        <div class="builder-controls">
          <div><label for="builderDraftModel">Draft Model</label><select id="builderDraftModel"><option value="">Default (from .env)</option></select></div>
          <div><label for="builderRefineModel">Refinement Model</label><select id="builderRefineModel"><option value="">Default (from .env)</option></select></div>
...
async function loadOpenRouterModels() {
    try {
        ...
        const draftSel = document.getElementById('builderDraftModel');
        const refineSel = document.getElementById('builderRefineModel');
        
        const models = data.data.sort((a,b) => a.id.localeCompare(b.id));
        for (const m of models) {
            const opt1 = document.createElement('option');
            opt1.value = m.id;
            opt1.textContent = m.id + ' ($' + (m.pricing.prompt*1e6).toFixed(2) + ' / M)';
            draftSel.appendChild(opt1.cloneNode(true));
            refineSel.appendChild(opt1);
        }
    } catch (e) {
